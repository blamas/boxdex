...
- **OG images** (`og-en.png`, `og-fr.png`, ...) are generated at build time by `src/pages/og-[locale].png.ts` (one static PNG per `LOCALES` entry), not committed to `public/`. Rendering is `satori` (JSX-less object tree in `src/lib/og-image.ts`) → SVG → `@resvg/resvg-js` → PNG. Two gotchas: satori has no native `<svg>` element, so the iso-cube icon (`public/favicon-iso.svg`, imported via `?raw`) is base64-encoded to a data URI for an `<img>`, and satori only reads TTF/OTF/WOFF (not WOFF2), so the font bytes come from `@fontsource/jetbrains-mono`'s `.woff` files via `createRequire(import.meta.url).resolve(...)` (plain `import.meta.resolve` isn't reliable under Astro's Vite SSR transform). Per-locale copy is `meta.titleRest`/`meta.ogDescription` in `src/i18n/locales/*.json` (same source as the rest of the UI strings), a leading space before French's `titleRest` colon must be U+00A0 (a plain space collapses under satori/CSS whitespace rules). `Layout.astro`/`index.astro` build the `og:image` URL as `${BASE}/og-${locale}.png`. `apple-touch-icon.png` (single file, no locale) follows the same generated-not-committed pattern: `src/pages/apple-touch-icon.png.ts` reads `public/favicon.svg` and rasterizes it straight through `@resvg/resvg-js` at 180x180, no satori needed since there's no text/layout.
