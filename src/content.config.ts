...
const compressionDriver = z.object({
...
// Source of truth for driver validation (enforced at build/`astro sync`). The committed
// editor-facing mirrors in schema/*.schema.json are generated from these zod schemas via
// `npm run schema:gen`. Regenerate after any change here, never hand-edit them.
const driverSchema = z.discriminatedUnion("type", [coneDriver, compressionDriver]);
...
// compression driver when its `exitInch` throat matches. Regenerate schema/horn.schema.json
// via `npm run schema:gen` after any change here.
